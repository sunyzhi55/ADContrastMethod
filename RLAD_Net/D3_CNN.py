...
class Base3DModel(nn.Module):
    def __init__(self, num_seg_classes, f=8):
        ...
        self.conv.add_module('conv1', nn.Conv3d(in_channels=1, out_channels=4 * f, kernel_size=1, stride=1, padding=0,
                                                dilation=1))
        ...
        self.conv.add_module('conv9',
                             nn.Conv3d(in_channels=32 * f, out_channels=64 * f, kernel_size=2, stride=1, padding=2,
                                       dilation=2))
        ...
        self.conv.add_module('conv13',
                             nn.Conv3d(in_channels=64 * f, out_channels=64 * f, kernel_size=2, stride=1, padding=1,
                                       dilation=2))
        ...
        self.fc.add_module('fc1', nn.Linear(110592, 1024))
        self.fc.add_module('dp1', nn.Dropout(0.3))
        self.fc.add_module('fc2', nn.Linear(1024, 2))

    def forward(self, x):
        z = self.conv(x)  # x [8, 1, 128, 128, 128] z [8, 512, 6, 6, 6]
        z = self.fc(z.view(x.shape[0], -1))
        # import pdb;
        # pdb.set_trace()
        # new = z.view(x.shape[0], 18432) # new[8. 18432]
        # z = self.fc(new)
