...
from RLAD_Net.Transformer import TransformerModel
from RLAD_Net.PositionalEncoding import FixedPositionalEncoding, LearnedPositionalEncoding
from RLAD_Net.Unet_skipconnection import Unet
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        x = torch.rand((4, 1, 128, 128, 128), device=cuda0)
        _, model = get_model()
        model.cuda()
        decoder_output, y, heatmap = model(x)
        ...
        """
        decoder_output torch.Size([2, 1, 128, 128, 128])
        y torch.Size([2, 2])
        heatmap torch.Size([2, 2, 16, 16, 16])
        """
